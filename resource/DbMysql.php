...
class MysqlDb
{
  ...
  public function updateTable($table, $entityValDic, $conjuctiveSelection){
      $entitiesArr = array_keys($entityValDic);
      $valArr = array_values($entityValDic);
      $arraySize = count($entityValDic);
      $sqlStr = "update $table set ";
      for($i =0; $i<$arraySize; $i++ ){
          if($i == ($arraySize -1)){
             $sqlStr .= $entitiesArr[$i] ." = ".$valArr[$i]; 
          }
          else{
               $sqlStr .= $entitiesArr[$i] ." = ".$valArr[$i].", "; 
          }          
      ...
      $sqlStr .= " where ";
      $selectEntities = array_keys($conjuctiveSelection);
      $selectVals = array_values($conjuctiveSelection);
      $numOfSelect = count($conjuctiveSelection);
      
    for($i=0; $i<$numOfSelect; $i++){
        if($i == ($numOfSelect - 1)){
          $sqlStr .= $selectEntities[$i]." = " .$selectVals[$i];
        }
        else{
          $sqlStr .= $selectEntities[$i]." = " .$selectVals[$i]+" and";
        }
    }  
    if($this-> link-> query($sqlStr))
    {
